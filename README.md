...
> macOS 是当前主要发布目标；Windows 后端已可从源码运行和打包，但候选面板 UI 仍在演进中。
...
在 macOS 上，程序使用辅助功能定位输入光标、显示候选面板并完成文本回写，因此必须授予辅助功能和输入监控权限。Windows 后端使用 Win32 Raw Input 捕获键盘事件，并用 SendInput 回写文本；它不需要 macOS 式隐私授权，但普通权限进程无法向管理员权限窗口注入文本。

## 当前限制

- macOS 是主要支持平台；Windows 支持仍处于实验阶段，当前没有原生候选面板浮窗。
...
macOS 开发环境需要 Rust、Homebrew 和 librime：
...
Windows 开发环境使用 Rust、LLVM、MSYS2 和 MSYS2 的 `librime`：
