...
impl RewriteTransactionGuard {
    ...
    #[cfg(any(target_os = "macos", target_os = "windows"))]
    fn begin() -> Self {
        platform::begin_rewrite_transaction();
        Self::new(platform::cancel_rewrite_transaction)
    }
...
#[cfg(any(target_os = "macos", target_os = "windows"))]
extern "C" fn platform_event_callback(event: platform::InputEvent) -> i32 {
...
impl ListenerRuntime {
    fn handle_event(&mut self, event: platform::InputEvent) -> Result<bool> {
        ...
        if event.event_type == platform::EVENT_CONTEXT {
            if event.is_rewrite_active() {
                if self.options.log_events {
        ...
        if event.has_command_modifier()
            && matches!(event.key_code, platform::KEY_TAB | platform::KEY_GRAVE)
        {
        ...
        match event.key_code {
            ...
            | platform::KEY_RETURN
            | platform::KEY_ESCAPE
            | platform::KEY_ARROW_LEFT
            | platform::KEY_ARROW_RIGHT
            | platform::KEY_ARROW_DOWN
            | platform::KEY_ARROW_UP => {
    ...
    fn handle_modified_key(&mut self, event: platform::InputEvent) -> Result<bool> {
        if event.has_control_modifier() && event.key_code == platform::KEY_W {
        ...
        if event.key_code == platform::KEY_BACKSPACE
            || (event.has_control_modifier() && event.key_code == platform::KEY_H)
        {
            self.clear_capture_context("unmodeled modified deletion");
            return Ok(false);
        }

        if event.has_control_modifier() && event.key_code == platform::KEY_C {
    ...
    fn handle_candidate_interaction(&mut self, key_code: u32, text: &str) -> Result<Option<bool>> {
        ...
        let is_space_selection = key_code == platform::KEY_SPACE && key == ' ';
    ...
    fn perform_rewrite(&self, delete_chars: usize, replacement_text: &str) -> Result<()> {
        ...
        platform::commit_rewrite_transaction(
            delete_chars,
            replacement_text,
            self.options.inject_delay_ms,
        )?;
    ...
    fn show_candidate_preview(&self, preview: &CandidatePreview) -> Result<()> {
        let candidates = self
            ...
            .collect::<Vec<_>>();
        platform::update_candidate_panel(
            &preview.preedit,
            &candidates,
...
fn event_input_source_fingerprint(event: &platform::InputEvent) -> String {
...
fn candidate_interaction_for_event(
    ...
    if key_code == platform::KEY_SPACE && key == ' ' {
...
fn is_pending_pinyin_commit_space(capture: &CaptureState, key_code: u32, text: &str) -> bool {
    key_code == platform::KEY_SPACE && text == " " && capture.pending_pinyin().is_some()
}

fn input_source_is_system(input_source: &str) -> bool {
    if input_source == "windows-system" {
        return true;
    }
...
mod tests {
    ...
    fn space_selects_first_candidate_on_current_page() {
        ...
        assert_eq!(
            candidate_interaction_for_event(&config, platform::KEY_SPACE, ' '),
            Some(CandidateInteractionKey::Select(0))
        );
        assert_eq!(
